...
const verifyAdmin = (req, res, next) => {
  // 1. ตรวจสอบว่า verifyToken ได้ดึงข้อมูล User มาฝังไว้ให้หรือยัง
  if (!req.user) {
    return res.status(401).json({ success: false, message: '❌ ไม่พบข้อมูลผู้ใช้งานในระบบ' });
  }

  // 2. เช็คสิทธิ์ (พี่สามารถเปลี่ยนคำว่า 'ADMIN' หรือ 'admin' ให้ตรงกับในฐานข้อมูล Prisma ของพี่ได้เลยครับ)
  if (req.user.role !== 'ADMIN' && req.user.role !== 'admin') {
    return res.status(403).json({ success: false, message: '❌ ปฏิเสธการเข้าถึง: สิทธิ์นี้เฉพาะระดับผู้จัดการคลังสินค้าเท่านั้น' });
  }

  // 3. ถ้าเป็น Admin จริง ก็ปล่อยให้ผ่านเข้าไปทำงานต่อได้เลย
  next();
};
// ==========================================================================
